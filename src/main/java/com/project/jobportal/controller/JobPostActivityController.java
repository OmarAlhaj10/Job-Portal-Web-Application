...
public class JobPostActivityController {
    ...
    @Autowired
    public JobPostActivityController(UsersService usersService, JobPostActiviyService jobPostActiviyService, JobSeekerApplyService jobSeekerApplyService, JobSeekerSaveService jobSeekerSaveService) {
    ...
    public String searchJobs(Model model,
        ...
        if (!dateSearchFlag && !remote && !type && !StringUtils.hasText(job) && !StringUtils.hasText(location)){
            jobPost = jobPostActiviyService.getAll();
        }else {
            jobPost = jobPostActiviyService.search(job, location, Arrays.asList(partTime, fullTime, freelance), Arrays.asList(remoteOnly, officeOnly, partialRemote), searchDate);
        }
        ...
        if (!(authentication instanceof AnonymousAuthenticationToken)) {
            ...
            if (authentication.getAuthorities().contains(new SimpleGrantedAuthority("Recruiter"))) {
                List<RecruiterJobsDto> recruiterJobs = jobPostActiviyService.getRecruiterJobs(((RectuiterProfile)currentUserProfile).getUserAccountId());
                model.addAttribute("jobPost", recruiterJobs);
            }else {
                List<JobSeekerApply> jobSeekerApplyList = jobSeekerApplyService.getCandidatesJobs((JopSeekerProfile) currentUserProfile);
    ...
    public String addNew(JobPostActivity jobPostActivity, Model model) {
        ...
        JobPostActivity saved = jobPostActiviyService.addNew(jobPostActivity);
